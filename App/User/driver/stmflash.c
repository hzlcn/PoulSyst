...
void STMFLASH_Erase(uint32_t addr)
{
	...
	/*擦除扇区(area defined by FLASH_USER_START_ADDR and FLASH_USER_END_ADDR) ***********/	
...
{
	u16 i;
	u16 halfWordSize = (NumToWrite >> 2) + ((NumToWrite & 0x03) ? 1 : 0);
	HAL_FLASH_Unlock();         //解锁
	for (i = 0; i < halfWordSize; i++) {
		HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, WriteAddr, *pBuffer);
		WriteAddr += 4;
		pBuffer++;
	}
	HAL_FLASH_Lock();           //上锁
